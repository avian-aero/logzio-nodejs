...
class LogzioLogger {
    ...
    async _send(bulk) {
        ...
        if (this.compress) {
            ...
            options.body = await zlibPromised(body);
        } else {
            options.body = body;
        }

        await this._tryToSend(options, bulk);
    }

    async _tryToSend(options, bulk) {
        try {
